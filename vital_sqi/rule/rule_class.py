...
class Rule:
    ...
    def __setattr__(self, name, value):
        if name == "name":
            if not isinstance(value, str) or not bool(
                re.match("^[A-Za-z0-9_-]*$", value)
            ):
                raise AttributeError(
                    "Name of SQI rule must be a string "
    ...
    def update_def(self, op_list, value_list, label_list):
        ...
        for op in op_list:
            if op not in ["<", "<=", ">", ">=", "="]:
                raise ValueError(
                    "Invalid operand: Expect string operands, "
                    "instead found {0}" + op + ", type {1}".format(op, type(op))
                )
        for value in value_list:
            if not np.isreal(value):
                raise ValueError(
                    "Invalid threshold: Expect numeric type "
                    "threshold, instead found {0}"
                    + str(value)
