...
def remove_invalid_smartcare(s, info, output_signal=True):
    ...
    if {"SPO2_PCT", "PERFUSION_INDEX", "PULSE_BPM"}.issubset(
        set(info.columns)
    ) is False:
        warnings.warn(
            "Info does not contain Smartcare columns. " "Using only signal to filter"
        )
    assert isinstance(output_signal, bool), "Expected a boolean value."
...
def trim_signal(s, sampling_rate, duration_left=300, duration_right=300):
    ...
    check_signal_format(s)
    assert (
        np.isreal(duration_right) or duration_right is None
    ), "Expected a numeric value or None"
    assert (
        np.isreal(duration_left) or duration_left is None
    ), "Expected a numeric value or None"
    ...
    if int((duration_left + duration_right) * sampling_rate * 2) > len(s):
        warnings.warn(
            "Trimming length exceeds the signal length. "
...
def interpolate_signal(s, missing_index, missing_len, method="arima", lag_ratio=10):
    ...
    check_signal_format(s)
    assert isinstance(
        missing_index, (list, tuple, np.array)
    ), "Expected a list or a np.array."
    assert isinstance(
        missing_len, (list, tuple, np.array)
    ), "Expected a list or a np.array."
    assert (
        isinstance(method, str) and method == "arima"
    ...
    for pos, number_of_missing_instances in zip(missing_index, missing_len):
        ...
        if method == "arima":
            ...
            fc, confint = model.predict(
                n_periods=number_of_missing_instances, return_conf_int=True
            )
