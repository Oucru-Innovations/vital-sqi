...
def save_segment(
    ...
    assert isinstance(segment_name, str) or (segment_name is None), "Expected a string."
    ...
    for segment in tqdm(segment_list):
        ...
        try:
            ...
            if save_image:
                ...
                fig = go.Figure()
                fig.add_traces(
                    go.Scatter(x=np.arange(1, len(segment)), y=segment, mode="lines")
                )
                fig.update_layout(autosize=True)
                fig.write_image(os.path.join(save_img_folder, saved_filename + ".png"))

            np.savetxt(
                os.path.join(save_file_folder, saved_filename + ".csv"),
                segment,
                delimiter=",",
            )  # as an array
...
def split_segment(
    ...
    assert np.isreal(duration) or duration is None, "Expected a numeric value or None"
    assert np.isreal(overlapping) or None, "Expected a numeric value or None"
    assert isinstance(peak_detector, int), "Expected an int value in range [" "0, 7]."
    assert 0 <= peak_detector <= 7, "Expected an int value in range [0, 7]."
    assert wave_type == "ppg" or wave_type == "ecg", (
        "Expected str of either " '"ppg" or "ecg".'
    )
    ...
    if split_type == 0:
        ...
        chunk_indices = [
            ...
            for i in range(0, len(s), chunk_size - chunk_step)
        ]
    else:
        if wave_type == "ppg":
            detector = PeakDetector(wave_type="ppg")
            peak_list, trough_list = detector.ppg_detector(
                s.iloc[:, 1], detector_type=peak_detector
            )
        else:
            detector = PeakDetector(wave_type="ecg")
            peak_list, trough_list = detector.ecg_detector(
                s.iloc[:, 1], detector_type=peak_detector
            )
        chunk_indices = []
        for i in range(0, len(peak_list), int(duration - overlapping)):
            if i + duration < len(peak_list):
                chunk_indices.append([peak_list[int(i)], peak_list[int(i + duration)]])
