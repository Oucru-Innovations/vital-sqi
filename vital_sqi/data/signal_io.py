...
def ECG_reader(
    ...
    """
    if file_type == "mit":
        assert file_type == "mit" and glob.glob(file_name + ".*"), "Files not found"
    else:
    ...
    assert not (
        channel_name is not None and channel_num is not None
    ...
    assert (
        ...
        or isinstance(sampling_rate, int)
    ...
    if file_type == "edf":
        ...
        if sampling_rate is None:
            try:
                ...
                print(
                    "sampling_rate is not defined and could not be "
        ...
        if start_datetime is None:
            try:
                ...
                print(
                    "start datetime is not defined and could not be "
                    "obtained from the signal's header."
                )
                pass
        else:
    ...
    if file_type == "csv":
        ...
        signals = pd.read_csv(file_name, usecols=use_cols, skipinitialspace=True)
        try:
            # start_datetime in datetime and column in second
            if start_datetime is not None and isinstance(signals.iloc[:, 0], float):
                start_datetime = pd.Timestamp(start_datetime)
                timestamps[0] = start_datetime
                timestamps = np.array(
                    start_datetime + pd.to_timedelta(timestamps, unit="seconds")
                )
...
def ECG_writer(signal_sqi, file_name, file_type, info=None):
    ...
    """
    signals = signal_sqi.signals.loc[
        :, signal_sqi.signals.columns != "timestamps"
    ].to_numpy()
    sampling_rate = signal_sqi.sampling_rate
    start_datetime = signal_sqi.start_datetime
    assert isinstance(sampling_rate, int) or isinstance(
        sampling_rate, float
        ...
        if info is not None:
            ...
            for i in range(len(annotations)):
                if isinstance(header["annotations"][i][1], bytes):
                    header["annotations"][i][1] = float(
                        str(header["annotations"][i][1], "utf-8")
                    )
            highlevel.write_edf(
                file_name,
                signals,
    ...
    if file_type == "csv":
        timestamps = generate_timestamp(start_datetime, sampling_rate, signals.shape[0])
...
def PPG_reader(
    ...
    if timestamp_unit is None:
        raise Exception(
            "Missing sampling_rate, not able to infer "
        ...
        raise Exception(
            "Timestamp unit must be either second (s) or " "millisecond (ms)"
        )
    ...
    signals.insert(0, "timestamps", timestamps)
    out = SignalSQI(
        signals=signals, wave_type="ppg", sampling_rate=sampling_rate, info=info
    )
    return out
