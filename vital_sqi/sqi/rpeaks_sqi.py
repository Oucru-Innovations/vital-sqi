...
def ectopic_sqi(
    ...
    with HiddenPrints():
        rr_intervals = get_nn(
            ...
            rpeak_method=rpeak_detector,
            remove_ectopic_beat=False,
        )

        rr_intervals_cleaned = remove_outliers(
            rr_intervals, low_rri=low_rri, high_rri=high_rri
        )
...
def msq_sqi(s, peak_detector_1=7, peak_detector_2=6, wave_type="ppg"):
    ...
    if wave_type == "ppg":
        ...
        peaks_2, trough_list_2 = detector.ppg_detector(
        ...
        peaks_2 = detector.ecg_detector(
            s, detector_type=peak_detector_2, preprocess=False
        )
    if len(peaks_1) == 0 or len(peaks_2) == 0:
        return 0.0
    elif len(peaks_1) != len(peaks_2):
        return np.abs(len(peaks_1) - len(peaks_2)) / np.mean(
            [len(peaks_1), len(peaks_2)]
        )
